...
export default function addzone() {
  ...
  const [tags, setTags] = useState(["Pakistan", "India", "USA"]);

  const onSubmit = async (data) => {
    console.log(data);
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_HOST}/api/addZone`,
      {
        method: "POST",
        headers: {
    ...
    if (data2.success == true) {
      ...
      toast({
        title: "error",
        description: "Duplicate Product Try Agaibn",
        status: "error",
        duration: 5000,
        isClosable: true,
      });
    ...
    <>
      ...
      >
        <Box bg={"#153A5B"} p="0.4rem">
          <Center>
            <Heading color={"white"}>Add Zone</Heading>
          </Center>
        </Box>
